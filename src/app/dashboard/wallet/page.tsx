...
const FinanceDashboard: React.FC = () => {
  ...
  return (
    <div className="p-8 pt-6 min-h-screen ">
      {/* Header */}
      ...
      <div className="grid grid-cols-12 gap-4">
        {/* Transaction table */}
        <div className="col-span-6 rounded-xl bg-background shadow-sm border border-[var(--color-border)]/10 p-5 transition-all hover:shadow-md">
          <h2 className="text-lg font-semibold mb-3 ">Lịch sử tiêu tiền</h2>
          <table className="w-full text-sm">
            <thead>
              <tr className="text-[var(--color-text)] border-b border-[var(--color-border)]/10">
                <th className="text-left pb-2">Tên giao dịch</th>
                <th className="text-left pb-2">Loại</th>
                <th className="text-left pb-2">Số tiền</th>
                <th className="text-left pb-2">Thời điểm</th>
              </tr>
            </thead>
            <tbody>
              {transactions.map((t) => (
                ...
                >
                  ...
                  <td>{t.category}</td>
                  <td className="text-right font-medium">{t.amount.toLocaleString()}đ</td>
                  <td>{t.date}</td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="col-span-6 grid grid-cols-2 gap-4">
          <div className="rounded-xl bg-background shadow-sm border border-[var(--color-border)]/10 p-5 flex flex-col items-center justify-center hover:shadow-md transition">
            <h2 className="text-lg font-semibold mb-3 ">Tỉ lệ chi tiêu</h2>
            <Chart options={pieOptions} series={pieSeries} type="pie" width="260" />
          </div>

          <div className="rounded-xl bg-background shadow-sm border border-[var(--color-border)]/10 p-5 hover:shadow-md transition">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-lg font-semibold ">Ngân sách </h2>
            </div>
                ...
                return (
                  <div key={b.id}>
                    <div className="flex justify-between mb-1">
                      ...
                      <span className="font-semibold">{b.spent.toLocaleString()}đ</span>
                    </div>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm text-[var(--color-text)]">{g.deadline}</span>
                </div>
                ...
                <p className="text-xs text-gray-500 italic">Ghi chú: {g.note}</p>
              </div>
            );
          })}
        </div>
