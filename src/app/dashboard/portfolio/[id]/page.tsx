        ...
        <div className="w-full min-h-screen p-4 sm:p-6">
            ...
            <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
                <div className='flex gap-2 items-center'>
                    ...
                    <h1 className="text-2xl sm:text-3xl font-bold text-text">My portfolio</h1>
                </div>
                <button
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 mb-4 sm:mb-6">
                ...
                <div className="lg:col-span-1 bg-background rounded-2xl p-4 sm:p-6 shadow-xl border border-text/10">
                    <h2 className="text-base sm:text-lg font-semibold text-text mb-4">Tổng tài sản</h2>
                    ...
                    {fundDetail.averageFinanceAssets && fundDetail.averageFinanceAssets.length > 0 ? (
                                ...
                                {fundDetail.averageFinanceAssets.map((asset, index) => (
                                    <div key={asset.assetName} className="flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            ...
                                            />
                                            <span className="text-xs sm:text-sm text-text">{asset.assetName}</span>
                                        </div>
                                        <span className="text-xs sm:text-sm font-medium text-text">{asset.percentageInPortfolio}%</span>
                                    </div>
                                ))}
                            </div>
                        </>
                        ...
                        <div className="flex flex-col items-center justify-center py-12">
                            <svg className="w-16 h-16 sm:w-20 sm:h-20 text-text/20 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="bg-background rounded-2xl p-4 sm:p-6 shadow-xl border border-text/10">
                        <p className="text-xs sm:text-sm text-text/60 mb-2">Tổng tiền quỹ hiện tại</p>
                        <p className="text-xl sm:text-2xl font-bold text-text">{Number(fundDetail.totalFinanceCurrent).toLocaleString("vi-VN")}</p>
                    </div>

                    {/* Total Transaction Amount - Số tiền vốn bỏ ra */}
                    <div className="bg-background rounded-2xl p-4 sm:p-6 shadow-xl border border-text/10">
                        <p className="text-xs sm:text-sm text-text/60 mb-2">Số tiền vốn bỏ ra</p>
                        <p className="text-xl sm:text-2xl font-bold text-text">{Number(fundDetail.totalTransactionAmount).toLocaleString("vi-VN")}</p>
                    </div>
                    ...
                    <div className="bg-background rounded-2xl p-4 sm:p-6 shadow-xl border border-text/10">
                        ...
                        <p className={`text-xl sm:text-2xl font-bold ${getChangeColor(fundDetail.totalProfitAndLoss)}`}>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            <thead className="bg-background border-b border-text/10">
                                <tr>
                                    <th className="px-3 sm:px-6 py-3 sm:py-4 text-left text-xs font-semibold text-text uppercase tracking-wider whitespace-nowrap">Tiền ảo</th>
                                    <th className="px-3 sm:px-6 py-3 sm:py-4 text-right text-xs font-semibold text-text uppercase tracking-wider whitespace-nowrap">Giá</th>
                                    <th className="px-3 sm:px-6 py-3 sm:py-4 text-right text-xs font-semibold text-text uppercase tracking-wider whitespace-nowrap">24g</th>
                                    <th className="hidden md:table-cell px-3 sm:px-6 py-3 sm:py-4 text-right text-xs font-semibold text-text uppercase tracking-wider whitespace-nowrap">7ng</th>
                                    <th className="hidden lg:table-cell px-3 sm:px-6 py-3 sm:py-4 text-right text-xs font-semibold text-text uppercase tracking-wider whitespace-nowrap">Vốn hóa thị trường</th>
                                    <th className="hidden xl:table-cell px-3 sm:px-6 py-3 sm:py-4 text-center text-xs font-semibold text-text uppercase tracking-wider whitespace-nowrap">7 ngày qua</th>
                                    <th className="px-3 sm:px-6 py-3 sm:py-4 text-center text-xs font-semibold text-text uppercase tracking-wider whitespace-nowrap">Hành động</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-text/5">
                                {fundDetail.listInvestmentAssetResponse.map((asset) => (
                                    <tr key={asset.idAsset} className="hover:bg-text/5 transition-colors">
                                        ...
                                        <td className={`px-3 sm:px-6 py-3 sm:py-4 text-right font-medium whitespace-nowrap text-xs sm:text-sm ${getChangeColor(asset.priceChangePercentage24h)}`}>
                                        ...
                                        <td className="px-3 sm:px-6 py-3 sm:py-4">
                                            <div className="flex items-center justify-center gap-1 sm:gap-2">
                                                ...
                                                >
                                                    <svg className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                                                    </svg>
                                                </button>
                                                <button 
