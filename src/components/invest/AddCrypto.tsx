...
const AddCrypto = ({ isOpen, onClose, onSubmit, loading, fundId }: AddCryptoModalProps) => {
                ...
                {step === 'select' ? (
                    ...
                    <>
                        <div className="p-8 pb-4 flex-shrink-0">
                            <h2 className="text-2xl font-bold text-text mb-6 text-center">Chọn tiền ảo</h2>
                                        ...
                                        >
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    />
                                                    <div>
                                                        <div className="font-semibold text-text">{crypto.name}</div>
                                                        <div className="text-xs text-text/60 uppercase">{crypto.symbol}</div>
                                                    </div>
                                                </div>
                                                <div className="text-right">
                                                    <div className="font-semibold text-text">{formatPrice(crypto.current_price)}</div>
                    ...
                    <div className="p-8">
                        ...
                        <h2 className="text-2xl font-bold text-text mb-6 text-center">Thêm {selectedCrypto?.name}</h2>
                            ...
                            <div className="mb-6 p-4 bg-background rounded-lg border border-text/10">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex-1">
                                        <div className="font-semibold text-text">{selectedCrypto.name}</div>
                                        <div className="text-sm text-text/60">{formatPrice(selectedCrypto.current_price)}</div>
                                    </div>
                                </div>
                            </div>
                        )}
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            <div className="p-4 bg-background rounded-lg border border-text/10">
                                <p className="text-text/70 text-sm mb-2">Bạn có chắc chắn muốn thêm tiền ảo này vào quỹ?</p>
                                <div className="text-text font-medium">{selectedCrypto?.name} ({selectedCrypto?.symbol.toUpperCase()})</div>
                            </div>
